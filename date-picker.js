...
const DatePicker = function (props) {
  ...
  function renderYearsOfDecade () {
    ...
    var rowCount = 0
    var year
    var startYear = (iter.getFullYear() - (iter.getFullYear() % 10))
  ...
  function render () {
    ...
    var decade = (selected.getFullYear() - (selected.getFullYear() % 10))
    ...
    props.i18n.days.map(function (day) { return yo`<th>${day}</th>` })
        ...
        <tbody>
          ${((props.viewMode === 'years' && renderYearsOfDecade()) || (props.viewMode === 'months' && renderMonthsOfYear()) || (props.viewMode === 'days' && renderDaysOfMonth())).map(function (row) {
